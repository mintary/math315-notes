...
\subsection{Generalization of resonance to higher-order ODEs}
...
\begin{mdframed}
\subsection{Steps to using complexification to solve for particular solutions}
\begin{enumerate}
    \item Complexify the equation by replacing the real input function with a complex exponential function. For example, if the input is $\cos(\omega t)$, we can replace it with $e^{i \omega t}$.
    \item Apply the exponential response formula to find a particular solution to the complexified equation.
    \item Extract the real part of the particular solution to get a particular solution to the original equation (if the original input was a cosine function) or extract the imaginary part to get a particular solution to the original equation (if the original input was a sine function).
\end{enumerate}
\end{mdframed}

\subsection{Polynomial inputs}
If we have a polynomial input, we can guess a particular solution that is also a polynomial of the same degree. For example, if we have:
\begin{equation*}
\ddot x + 4x = t^2
\end{equation*}
...
Then we can compute $\dot x_p$ and $\ddot x_p$:
\begin{align*}
\dot x_p &= 2At + B \\
\ddot x_p &= 2A
\end{align*}

Then we can plug this into the ODE to get:
\begin{align*}
2A + 4(At^2 + Bt + C) &= t^2 \\
(4A) t^2 + (4B) t + (2A + 4C) &= t^2
\end{align*}
...
2A + 4C &= 0
\end{align*}
which gives us $A = \frac{1}{4}$, $B = 0$, and $C = -\frac{1}{8}$. Thus, the particular solution is:
\begin{equation*}
\boxed{
x_p(t) = \frac{1}{4} t^2 - \frac{1}{8}
}
\end{equation*}

\thm{Undetermined coefficients}{
    Given:
    \[
        p(D)x(t) = q(t)
    \]
    where $q(t)$ is a $k$-th degree polynomial, and that $p(0) \not = 0$, then there exists exactly one solution which is a polynomial of degree less than or equal to $k$. 
}

Note that $p(0) = 0$ implies that $Ia_0 = 0$, or the ODE only involves derivatives. If this is the case, then this theorem may not apply.

\subsection{Reduction of order}

Second-order ODEs may also have the form:
\begin{equation*}
p(t) \ddot x + q(t) \dot x + r(t) x = 0
\end{equation*}
...
s(s + 1) = 0 
    \end{align*}
which gives us the roots $r = 0$ and $r = -1$. Thus, the homogeneous solution is:
    \begin{equation*}
x_h(t) = C_1 + C_2 e^{-t}
    \end{equation*}

    We can use a substitution $u = \dot x $ to reduce the order of the ODE. Then we have $\dot u = \ddot x$, so we can rewrite the ODE as:
\begin{equation*}
\dot u + u = t
\end{equation*}

Then if we guess $u = At + B$, and $\dot u = A$, we can plug this into the ODE to get:
\begin{align*}
A + At + B &= t \\
A &= 1 \\
B &= -1
...
Thus, we have $u(t) = t - 1$, and so $x(t) = \int u(t) dt = \int (t - 1) dt = \frac{t^2}{2} - t + C_3$. We just let $C_3 = 0$ since it is already captured by the homogeneous solution. Thus, the particular solution is:
\begin{equation*}
x_p(t) = \frac{t^2}{2} - t
\end{equation*}
...
We might obtain an equation like:
\begin{equation*}
3 \ddot x + 8 \dot x + 6x = (t^2 + 1) e^{-t}
\end{equation*}

Let us suppose some function $u(t)$ exists such that $x_p(t) = u(t) e^{-t}$ is a particular solution to the above equation. Then we can compute $\dot x_p$ and $\ddot x_p$:
\begin{align*}
\dot x_p &= \dot u e^{-t} - u e^{-t} \\
\ddot x_p &= \ddot u e^{-t} - 2 \dot u e^{-t} + u e^{-t}
\end{align*}

Then we can plug this into the ODE to get:
\begin{align*}
3 (\ddot u e^{-t} - 2 \dot u e^{-t} + u e^{-t}) + 8 (\dot u e^{-t} - u e^{-t}) + 6 (u e^{-t}) &= (t^2 + 1) e^{-t} \\
3 \ddot u e^{-t} + 2 \dot u e^{-t} + u e^{-t} &= (t^2 + 1) e^{-t}
\end{align*}

Then we have:
\begin{equation*}
3\ddot u + 2 \dot u + u = t^2 + 1
\end{equation*}

To solve this, we guess $u = At^2 + Bt + C$, and then we can compute $\dot u$ and $\ddot u$:
\begin{align*}
\dot u &= 2At + B \\
\ddot u &= 2A
\end{align*}
...
Then we can plug this into the ODE to get:
\begin{align*}
3(2A) + 2(2At + B) + (At^2 + Bt + C) &= t^2 + 1 \\
6A + 4At + 2B + At^2 + Bt + C &= t^2 + 1
\end{align*}

Grouping by powers of $t$ gives:
\begin{align*}
At^2 + (4A + B) t + (6A + 2B + C) &= t^2 + 1
\end{align*}
...
We get that $A = 1$, $B = -4$, and $C = 3$.

So we have $u(t) = t^2 - 4t + 3$, and thus the particular solution is:
\begin{equation*}
x_p(t) = u(t) e^{-t} = (t^2 - 4t + 3) e^{-t}
\end{equation*}
...
\nt{
    We often end up with a polynomial multiplied by an exponential. To evaluate an integral involving this expression more easily, you can apply the operator trick.
    \begin{equation*}
        \boxed{
            \int P(t) e^{\alpha t} dt = e^{\alpha t} \left( \frac{P(t)}{\alpha} - \frac{P'(t)}{\alpha^2} + \frac{P''(t)}{\alpha^3} - \cdots \right)
        }
    \end{equation*}
    This works because when we do integration by parts, we continue until we get a derivative equal to zero, which just returns a constant. We can also apply this operator trick to solve ODEs. For example, if we have:
    \begin{equation*}
    (D - \alpha I) x = P(t) e^{\alpha t}
    \end{equation*}

    Then we can apply the operator trick to get:
    \begin{equation*}
    x = \frac{P(t)}{\alpha} e^{\alpha t} - \frac{P'(t)}{\alpha^2} e^{\alpha t} + \frac{P''(t)}{\alpha^3} e^{\alpha t} - \cdots
    \end{equation*}

    Note that this is similar to how we got the generalized exponential response formula for the case of resonance, where we had to take derivatives of the characteristic polynomial until we got a non-zero value.
}
